...
@section('content')
                ...
                <div class="col-md-12">
                    <div class="box box-primary">
                        <div class="box-header with-border">
                            <h3 class="box-title">Bank Entry Lists</h3>
                        </div>
                        <table id="example1" class="table table-bordered table-striped">
                            ...
                            <tbody>
                            <?php $i = 1; ?>
                            @foreach($bank_account_list as $b)
                                <tr>
                                    <td>{{$i++}}</td>
                                    <td>{{ $b->date}}</td>
                                    <td>{{ $b->particular}} </td>
                                    ...
                                    <td>
                                        <div class="dropdown">
                                            ...
                                            <ul class="dropdown-menu">
                                                <li>
                                                    <a href="{{route('admin.user.bank_account.list.edit',['id'=> $b->id, 'token'=>$token])}}"
                                                       data-id="{{ $b->id}}" class="btn-edit">Edit</a>
                                                </li>
    ...
    <script>
        ...
        $('#delete_cash_record').on('submit', function (e) {
            e.preventDefault();
            $.ajax({
                url: "{{route('admin.user.bank_account.list.delete')}}",
                method: "GET",
